...
def return_somethingv2(modality):
    ...
    if modality == 'RGB':
        root_data = ROOT_DATASET + 'Frames'
        filename_imglist_train = 'data/train_videofolder_mini_retrieved.txt'
        filename_imglist_val = 'data/val_videofolder_mini_retrieved.txt'
        prefix = '{:05d}.jpg'
    else:
        raise NotImplementedError('no such modality:'+modality)
    return filename_categories, filename_imglist_train, filename_imglist_val, root_data, prefix
...
def return_ntu(modality):
    filename_categories = 'data_fixed/category_mini.txt'
    if modality == 'RGB':
        root_data = ROOT_DATASET + 'Frames/ntu60_hrnet.pkl' 
        filename_imglist_train = 'data_fixed/train_videofolder_retrieved.txt' #datasets/ntu60/data/train_videofolder_retrieved.txt
        filename_imglist_val = 'data_fixed/val_videofolder_retrieved.txt'
        #prefix = '{:05d}.jpg'
        prefix = None
    else:
        raise NotImplementedError('no such modality:'+modality)
...
def return_dataset(dataset, modality):
    ...
    dict_single = {'jester': return_jester, 'something': return_something, 'somethingv2': return_somethingv2,
                   ...
                   'kinetics': return_kinetics, 'charades_full':return_charades_full , 'ntu':return_ntu }
    dict_charades = {'charades_ego': return_charades_ego}
    if dataset in dict_single:
        file_categories, file_imglist_train, file_imglist_val, root_data, prefix = dict_single[dataset](modality)
        print("Done 1.... ", file_imglist_train)
        ...
        print("Done 1.... ", file_imglist_train_1p)
