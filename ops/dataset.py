...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file, unlabeled= False,second_segments=2,
        ...
        #with open(self.root_path + os.sep +'ntu60_hrnet.pkl', 'rb') as f:
        #    self.list_of_ann_dic = pickle.load(f)['annotations']
    ...
    def _load_skl(self, directory, idx):

        skl_vid = next(item for item in self.list_of_ann_dic if item["frame_dir"] == directory)

        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            try:
                return [skl_vid['keypoint'][:,idx,:,:]]
            except Exception:
                print('error loading image:', os.path.join(self.root_path, directory, self.image_tmpl.format(idx)))
                return [skl_vid['keypoint'][:,idx,:,:]]
                #return [Image.open(os.path.join(self.root_path, directory, self.image_tmpl.format(1))).convert('RGB')]
    ...
    def __getitem__(self, index):
        ...
        return self.get_img(record, segment_indices)

    def get_skl(self, record, indices):
        ...
        for seg_ind in indices:
            ...
            for i in range(self.new_length):
                ...
                seg_imgs = self._load_skl(record.path, p)
    ...
    def get_img(self, record, indices):
