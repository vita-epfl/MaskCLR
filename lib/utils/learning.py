...
def load_pretrained_weights(model, checkpoint):
    ...
    if 'target_encoder' in checkpoint:
        ...
        print("loading targe encoder pretrained weights")
...
def load_backbone(args):
    ...
    if hasattr(args, "eval_backbone"):
        print("initialized target encoder backbone")
        model_backbone = DSTformerEncoder(dim_in=3, dim_out=3, dim_feat=args["e_dim_feat"], dim_rep=args["dim_rep"], 
        ...
        print("Instantiating backbone DSTformer")
        model_backbone = DSTformer(dim_in=3, dim_out=3, dim_feat=args["dim_feat"], dim_rep=args["dim_rep"], 
                                   depth=args["depth"], num_heads=args["num_heads"], mlp_ratio=args["mlp_ratio"], norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                                   maxlen=args["maxlen"], num_joints=args["num_joints"])
    
    elif args["backbone"]=='DSTformerv2':

        print("Instantiating backbone DSTformerv2" )
