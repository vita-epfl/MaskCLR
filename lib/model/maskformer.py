...
class Attention(nn.Module):
    ...
    def forward(self, x, seqlen=1):
        B, N, C = x.shape  # torch.Size([1944, 17, 256]) -> torch.Size([4, 3762, 512])
        ...
        if self.mode == 'series':
            ...
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
            x, attn = self.forward_spatial(q, k, v)
        else:
    ...
    def forward_spatial(self, q, k, v):
        ...
        #print("Spatial Attention: ", q.shape)
        ...
        #print("Spatial Attention: ", attn.shape) # Spatial Attention:  torch.Size([972, 8, 17, 17])
        
        ##print(torch.sum(x[0], axis=1).softmax(dim=-1))
        return x, attn
        
    def forward_temporal(self, q, k, v, seqlen=8):

        B, _, N, C = q.shape  ## torch.Size([1944, 8, 17, 32]) -> torch.Size([4, 8, 3536, 64])

        #print("Temporal Attention: ", q.shape)
        ##print(q.shape, seqlen)
        ...
        #print("Temporal Attention: ", attn.shape)  #Temporal Attention:  torch.Size([4, 8, 17, 243, 243])
...
class Block(nn.Module):
    ...
    def forward(self, x, seqlen=1):
        ...
        if self.st_mode=='stage_st':
            ...
            attn_s, attn_map_s = self.attn_s(self.norm1_s(x), seqlen)

            x = x + self.drop_path(attn_s)
            x = x + self.drop_path(self.mlp_s(self.norm2_s(x)))

            attn_t, attn = self.attn_t(self.norm1_t(x), seqlen)
            x = x + self.drop_path(attn_t)
            ...
            attn_s, attn = self.attn_s(self.norm1_s(x), seqlen)
...
class DSTformerv2(nn.Module):
    ...
    def attention_guided_random_masking(self, x, attention_map, mask_ratio, tau):
        """
    ...
    def forward(self, x, return_rep=False, attention_map=None, mask_ratio=0.5, tau=0.75):   
        ...
        #print("In shape: ", x.shape)   # In shape:  torch.Size([1944, 17, 256])
        ...
        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            ...
            attn = torch.mean(attn, axis=1).detach() # mean over the 8 heads
            
            
            ##print(attn_maps.shape, attn.shape)
            attn_maps = attn

            x_ts, attn = blk_ts(x, F)
            if self.att_fuse:
        ...
        #attn_maps = attn_maps.reshape(B, F, J, -1)
